...
class OverlaySyncService extends EventEmitter {
  private state: OverlaySyncState = {
   ...
   */
  isHealthy(): boolean {
    if (!this.state.isOverlayActive) return true; // Not active = not unhealthy
   ...
   */
  private handleStaleOverlay(): void {
    if (this.state.isStale) return; // Already handling
   ...
   * Log sync event
   */
  private logEvent(event: OverlaySyncEvent, message: string): void {
    logger.debug('OverlaySync', `[OVERLAY_SYNC] event=${event} ${message}`, {
...
// Singleton instance
let overlaySyncService: OverlaySyncService | null = null;

export function getOverlaySyncService(): OverlaySyncService {
...
export const OVERLAY_SYNC_CHANNELS = {
