...
class TimerEngine extends events_1.EventEmitter {
    ...
    startNextActivity() {
        if (!this.state.isWaitingForNextActivity || !this.state.waitingNextPhase) {
            logger_1.default.debug('TimerEngine', 'startNextActivity: nothing waiting');
            return false;
        }
        const phase = this.state.waitingNextPhase;
        this.state.isWaitingForNextActivity = false;
        this.state.waitingNextPhase = null;
        logger_1.default.info('TimerEngine', 'startNextActivity: starting queued phase', { phase });
        this.startPhase(phase);
        this.emitTick();
        this.saveState();
        return true;
    ...
    countsToCumulativeWorkTime(phase) {
        ...
        if (phase === 'custom' && (0, flowUtils_1.isFlowBasedSchedule)(schedule) && schedule.flowSteps) {
            const currentIndex = this.state.currentFlowStepIndex ?? 0;
     ...
     */
    triggerPendingBreakNow() {
        if (!this.state.isPostponed || !this.state.postponedPhase) {
            logger_1.default.debug('TimerEngine', 'triggerPendingBreakNow: no pending break to trigger');
            return false;
        }
        const pendingBreak = this.state.postponedPhase;
        logger_1.default.info('TimerEngine', 'triggerPendingBreakNow: triggering pending break immediately', {
            pendingBreak,
    ...
    resume() {
        ...
        if (this.state.isPostponed && this.state.postponedUntil && pausedDurationMs > 0) {
            ...
            logger_1.default.debug('TimerEngine', 'Resume: extended postponedUntil by paused duration', {
                pausedDurationMs,
    ...
    */
    skipPhase() {
        ...
        if (this.state.isWaitingForNextActivity && this.state.waitingNextPhase) {
            if (this.currentSchedule.noSkipEnabled)
                return;
            logger_1.default.info('TimerEngine', 'skipPhase: skipping queued waiting phase', {
    ...
    emitTick() {
        ...
        const tick = {
            ...
            ...(schedule && (0, flowUtils_1.isFlowBasedSchedule)(schedule) && currentIndex !== undefined
    ...
    getPostponeOptionsForCurrentPhase() {
        ...
        if (this.state.currentPhase === 'custom')
            return [];
        const breakType = (0, overlayPolicy_1.phaseToBreakType)(this.state.currentPhase);
        if (!breakType)
    ...
    getStrictModeForCurrentPhase() {
        ...
        if (this.state.currentPhase === 'custom' && (0, flowUtils_1.isFlowBasedSchedule)(schedule) && schedule.flowSteps) {
            const currentIndex = this.state.currentFlowStepIndex ?? 0;
            const step = schedule.flowSteps[currentIndex];
            if (step && step.type === 'custom') {
