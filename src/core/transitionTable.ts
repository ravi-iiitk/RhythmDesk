...
// Types are documented in definitions but not directly used at runtime
// import { PhaseType, SessionState, FlowStep } from '../shared/types';
...
export interface TransitionDefinition {
  name: string;
  category: 'automatic' | 'user' | 'system';
  
  // Preconditions that must be true before transition
  preconditions: string[];
  
  // State changes that result from transition
  postconditions: string[];
  
  // Side effects (overlay, persistence, events)
  sideEffects: string[];
...
export const TRANSITION_TABLE: Record<string, TransitionDefinition> = {
  // ============================================================
...
export function getTransitionsByCategory(
  ...
  return Object.values(TRANSITION_TABLE).filter(t => t.category === category);
}

/**
 * Log transition with definition context
 */
export function logTransitionWithDefinition(
  transitionName: string,
  context: Record<string, unknown>
): void {
  const definition = TRANSITION_TABLE[transitionName];
  if (definition) {
