...
export type BreakPhaseType = 'short-break' | 'long-break';

/**
 * Break priority levels (higher number = higher priority)
 */
export const BREAK_PRIORITY: Record<BreakPhaseType, number> = {
  'short-break': 1,
  'long-break': 2,
};
 ...
 */
export function isBreakPhase(phase: PhaseType): phase is BreakPhaseType {
 ...
 */
export function getBreakPriority(phase: PhaseType): number {
  if (isBreakPhase(phase)) {
    return BREAK_PRIORITY[phase];
  }
  return 0;
}

/**
 * Current break state for conflict resolution
 */
export interface BreakState {
  activeBreak: BreakPhaseType | null;     // Currently active break phase
  pendingBreak: BreakPhaseType | null;    // Pending (postponed) break
  pendingBreakDueAt: number | null;       // When pending break should trigger
}

/**
 * Result of break conflict resolution
 */
export interface BreakConflictResult {
  action: 'allow' | 'skip' | 'replace' | 'merge';
  reason: string;
  shouldClearPending?: boolean;           // Clear the pending break
  replacePendingWith?: BreakPhaseType;    // Replace pending with this break
}
 ...
 */
export function resolveBreakConflict(
  currentState: BreakState,
  newBreak: BreakPhaseType
): BreakConflictResult {
  ...
  if (activeBreak === newBreak) {
    logger.info('BreakConflict', 'Skipping duplicate active break', {
      activeBreak,
      newBreak,
    });
    return {
  ...
  if (activeBreak !== null) {
    ...
    if (newPriority > activePriority) {
      ...
      logger.warn('BreakConflict', 'Higher priority break while in break', {
        activeBreak,
        newBreak,
      });
      return {
  ...
  if (pendingBreak !== null) {
    ...
    if (pendingBreak === newBreak) {
      logger.info('BreakConflict', 'Same-type break already pending, merging', {
        pendingBreak,
        newBreak,
      });
      return {
    ...
    if (newPriority > pendingPriority) {
      logger.info('BreakConflict', 'Higher priority break supersedes pending', {
        pendingBreak,
        newBreak,
      });
      return {
    ...
    logger.info('BreakConflict', 'Lower priority break skipped', {
      pendingBreak,
      newBreak,
    });
    return {
 ...
 */
export function shouldClearPendingOnBreakEntry(
  pendingBreak: BreakPhaseType | null,
  enteringBreak: BreakPhaseType
 ...
 */
export function shouldSkipFlowBreakStep(
  pendingBreak: BreakPhaseType | null,
  flowStepBreak: BreakPhaseType
 ...
 */
export function validateBreakState(state: BreakState): string[] {
  const errors: string[] = [];
